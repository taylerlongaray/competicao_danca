...
if modo == "Painel do Jurado":
    if st.session_state.jurado_logado is None:
        ...
        if st.session_state.categoria_selecionada is None:
            ...
            # Card 1: Banner da Categoria e Fase
            st.markdown(
                f"""
            ...
            if not competidores_qualificados:
                ...
                # --- BOTÕES LATERALMENTE ALINHADOS AO CARTÃO DO COMPETIDOR ---
                col_btn_esq, col_nome_centro, col_btn_dir = st.columns([1, 6, 1])
                ...
                with col_nome_centro:
                    st.markdown(
                        f"""
                        <div class="jj-card jj-avaliando" style="margin-bottom: 0; padding: 10px 12px; text-align: center;">
                        ...
                        try:
                            ...
                            if not (1 <= val_nota <= 10):
                                ...
                                if st.session_state.idx_crit + 1 < total_crit:
                                    ...
                                    st.toast(
                                        f"✨ Nota registrada para {competidor_escolh_id} —"
                                        f" {criterio_nome}"
                                    )
                                else:
                                    st.session_state.idx_crit = 0
